...
bool DiskCacheFOZFile::WriteBlob(const foz_payload_key& Key, std::span<const std::span<const uint8_t>> BlobChunks, uint64_t& OutBlobOffset) {
  ...
  foz_payload_header ScratchHeader {.payload_size = (uint32_t)TotalBlobSize,
...
bool DiskCacheIndexedDB::StoreCacheBlob(const foz_payload_key& Key, std::span<const std::span<const uint8_t>> BlobChunks, DiskCacheIndex& CacheIndex) {
  ...
  mesa_index_db_file_entry IndexEntry {.hash = Hash,
...
std::optional<DiskCacheCodeHitData> DiskCache::Lookup(Core::InternalThreadState* Thread, const ExecutableFileSectionInfo& Region, uint64_t GuestRIP) {
  ...
  SizeNeeded += Header.SmallRelocCount * sizeof(DiskCacheBlobSmallRelocation) +
                Header.ThunkRelocCount * sizeof(DiskCacheBlobThunkRelocation) + Header.TouchedGuestPagesCount * sizeof(int64_t);
  if (Entry.Size < SizeNeeded) {
    return std::nullopt;
  }

  HitData.HostCode = {HitData.Blob.data() + sizeof(Header), Header.HostSize};
  HitData.EntryPoints = {reinterpret_cast<const DiskCacheBlobEntryPoint*>(HitData.Blob.data() + sizeof(Header) + Header.HostSize),
                         Header.EntryPointCount};

  auto* SmallRelocs = reinterpret_cast<const DiskCacheBlobSmallRelocation*>(
    HitData.Blob.data() + sizeof(Header) + Header.HostSize + Header.EntryPointCount * sizeof(DiskCacheBlobEntryPoint));
  auto* ThunkRelocs = reinterpret_cast<const DiskCacheBlobThunkRelocation*>(
    reinterpret_cast<const uint8_t*>(SmallRelocs) + Header.SmallRelocCount * sizeof(DiskCacheBlobSmallRelocation));
  ...
  auto* PageOffsets = reinterpret_cast<const int64_t*>(
    reinterpret_cast<const uint8_t*>(ThunkRelocs) + Header.ThunkRelocCount * sizeof(DiskCacheBlobThunkRelocation));
